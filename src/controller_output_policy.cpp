...
bool controller_output_policy_normalize_classic_rumble_stop_payload(
    ...
    // Classic rumble and actuator audio are mutually exclusive controller modes. A game's
    // zero-motor stop can leave the controller in rumble mode, so hand ownership back --
    // but only while Audio Haptics is actually running; otherwise the host's choice stands.
    payload[kValidFlag0Offset] = static_cast<uint8_t>(
