...
inline bool fill_output_report_checksum(uint8_t* outputData, size_t len)
{
    if (outputData == nullptr || len < 4) {
        return false;
    }

    const uint32_t crc = crc32(outputData, len - 4);
    outputData[len - 4] = static_cast<uint8_t>((crc >> 0) & 0xFF);
    outputData[len - 3] = static_cast<uint8_t>((crc >> 8) & 0xFF);
    outputData[len - 2] = static_cast<uint8_t>((crc >> 16) & 0xFF);
    outputData[len - 1] = static_cast<uint8_t>((crc >> 24) & 0xFF);
...
inline bool fill_feature_report_checksum(uint8_t *data, const size_t len) {
    if (data == nullptr || len < 4) {
        return false;
    }

    const uint32_t crc = crc32_feature(data, len - 4);
    data[len - 4] = static_cast<uint8_t>((crc >> 0) & 0xFF);
    data[len - 3] = static_cast<uint8_t>((crc >> 8) & 0xFF);
    data[len - 2] = static_cast<uint8_t>((crc >> 16) & 0xFF);
