...
#ifndef KITSUNE_ENABLE_HOST_ENCODER_PATH
#define KITSUNE_ENABLE_HOST_ENCODER_PATH 0
#endif
...
#if KITSUNE_ENABLE_HOST_ENCODER_PATH
...
#else
static inline bool host_pcm_iso_mounted(void) {
    return false;
}

static inline void host_pcm_iso_set_enabled(bool enabled) {
    (void)enabled;
}

static inline void host_pcm_iso_reset_stream(void) {
}

static inline bool host_pcm_iso_write(int16_t const *samples, uint16_t frames, uint32_t timestamp_us) {
    (void)samples;
    (void)frames;
    (void)timestamp_us;
    return false;
...
static inline uint32_t host_pcm_iso_drop_count(void) {
    return 0;
}
#endif
