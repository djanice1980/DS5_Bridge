...
// The USB mic contract is persona-shaped: DualSense Edge presents a stereo capture (both
// channels carry the same mono controller mic), every other persona presents mono. Buffers
// are sized for the stereo max; the live packet size follows the active persona.
...
void __not_in_flash_func(audio_loop)() {
    ...
    // Same reasoning as the input path: every tud_audio_* call below goes through TinyUSB's
    // endpoint mutex, which is null whenever the stack is not initialised, and the ready
    // predicates do not tell you that.
