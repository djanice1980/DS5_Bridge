...
// Adopted verbatim from upstream (SundayMoments/DS5_Bridge, v1.7.0) so the fork's deadzone is
// wire- and behaviour-compatible with upstream's CommandSetRadialDeadzones. The fork's own
// earlier implementation was also radial with the same rescale curve; upstream's byte mapping
// is exact at the endpoints (127 up / 128 down), which ours approximated within one count.
